...
describe('Shenzhen Solitaire Store', () => {
  ...
  it('collects dragons', () => {
    const dragons: Array<Card> = [0, 1, 2, 3].map((i) => ({
      id: `dragon-green-${i}`,
      kind: 'dragon',
      color: 'green',
    }))
    ...
    const lockedCell = state.freeCells.find(
      (c) => c?.kind === 'dragon' && c.isLocked,
    )
